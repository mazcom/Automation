...
foreach (var testFile in testFiles)
{
  ...
  using (StreamReader sr = new(testFile))
  {
    ...
    foreach (JObject jsonObject in jsonObjects)
    {
      ...
      if (jsonObject.Property("issue")! != null)
      {
        continue;
      }
      ...
      string bug;
      if (IssueHelper.CheckContainsBug(name, out bug) || IssueHelper.CheckContainsBug(description, out bug))
      {
        jsonObject.Property("description")!.AddAfterSelf(new JProperty("issue", bug));
        testProcessed++;
        wasConverted = true;
      }
    }
